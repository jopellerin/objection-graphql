...
class SchemaBuilder {
  ...
  getGraphQlFields(modelData) {
    return jsonSchemaUtils.jsonSchemaToGraphQLFields(modelData.modelClass.jsonSchema, {
  ...
  getRelationExtras(relation) {
    const fields = _.map(relation.joinTableExtras, 'joinTableCol');
    if (!fields) {
      return {};
    }

    return jsonSchemaUtils.jsonSchemaToGraphQLFields({
      properties: fields.reduce((carry, field) => ({ [field]: { type: 'string' } }), {})
    });
  }
  ...
  _typeForModel(modelData, relation = null) {
    ...
    let extras = {};
    if (relation) {
      const relationExtras = this.getRelationExtras(relation);
