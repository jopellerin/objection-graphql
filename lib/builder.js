...
const { jsonSchemaToGraphQLFields, jsonSchemaToGraphQLInput } = require('./jsonSchema');
...
class Builder {
  ...
  build() {
    ...
    if (!_.isEmpty(mutations)) {
      mutationTypes = new GraphQLObjectType({
        name: 'Mutation',
        fields: () => mutations,
      });
    }
  ...
  _buildGraphQlMutations(models) {
    ...
    _.forOwn(models, (modelData) => {
      ...
      if (useUpdate !== false) {
        ...
        mutations[updateMutationName] = this._mutationUpdate(updateMutationName, modelData);
      }

      if (useDelete !== false) {
        const deleteMutationName = `delete${_.upperFirst(singleFieldName)}`;
        mutations[deleteMutationName] = this._mutationDelete(deleteMutationName, modelData);
      }
    });
  ...
  _inputType(name, modelData, options = {}) {
    ...
    const { jsonSchema } = modelData.modelClass;
    const fields = jsonSchemaToGraphQLInput(jsonSchema, {
      include: modelData.opt.include,
      exclude: modelData.opt.exclude,
      typeNamePrefix: utils.typeNameForModel(modelData.modelClass),
      typeCache: this.typeCache,
